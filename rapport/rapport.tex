...
\begin{align}
    % 1. Projections brutes
    H_{x,n}(i) = \sum_{j} I_n(i, j) \quad &; \quad H_{y,n}(j) = \sum_{i} I_n(i, j) \\
    \nonumber \\
    % 2. Définition explicite de la normalisation (Chapeau)
    \hat{H}_{x,n}(i) = \frac{H_{x,n}(i)}{\sum_{i} H_{x,n}(i)} \quad &; \quad \hat{H}_{y,n}(j) = \frac{H_{y,n}(j)}{\sum_{j} H_{y,n}(j)} \\
    \nonumber \\
    % 3. Mesures de similarité
    Coef_Bhattacharyya(\hat{H}_n, \hat{H}_{n-1}) &= \sum_{k} \sqrt{\hat{H}_n(k) \cdot \hat{H}_{n-1}(k)} \\
    Distance_Bhattacharyya(\hat{H}_n, \hat{H}_{n-1}) &= \sqrt{1 - BC(\hat{H}_n, \hat{H}_{n-1})}
\end{align}
...
La conclusion logique pour cette métrique est assez décalée par rapport à la réalité terrain et ne représente pas la bonne stabilité. Son score même si proche de la réalité, est davantage de l'ordre de la coincidence que de la performence de la segmentation.
\newpage
...
Elle semble prometteuse en termes de performences et ne nécessessite même plus des joints de \texttt{NLF} comme donnée d'entrée, un prompt permet de détourer automatiquement l'objet étudié.
