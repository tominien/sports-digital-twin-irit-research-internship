...
Ce TER s'incrit dans une démarche plus générale, qui est de déterminer analytiquement la présence de montages par IA, notemment connus sous le nom de deepfakes.

\rapportfigure{deepfake_exemple_1.png}{0.50}{Exemple de deepfake (réinsertion de visage)}{fig:reinsertion}
\rapportfigure{deepfake_exemple_2.png}{0.50}{Exemple de deepfake (compositions d'élements)}{fig:reinsertion}
...
L'objectif de ce TER n'est pas de repérer directement les manipulations et de relever les inconstitences prouvant que la vidéo est fausse, mais de s'intéresser à la technique de manipulation de vidéo principale qui est la segmentation vidéo.
...
En effet l'enjeu est à la fois technique : réaliser la jonction entre différents outils déjà existant (nous étudierons notamment \texttt{MMPose}, \texttt{NLF} et différentes versions de Segment Anything aussi dit \texttt{SAM}) ; mais aussi méthodologique : quels enchainement d'outils permettent le meilleur ratio temps/performence ?

Le second point est la stratégie d'utilisations de ces outils pour créer une segmentation fiable et consistante de la vidéo originale. Ce critère est en phase avec la méthodologie, de mauvaises performences ou la découverte de nouveau outils reconduisent à la première tâche.
...
Nous avons choisi d'utiliser principalement les infrastructures de google colab, pour leur persistence selon les divers supports, étant donné que le matériel est uniquement géré par google.
...
Ces versions représentent une amélioration notable et en rupture par rapport à leur précédente.

\subsection{Version V1 : étude de \texttt{NLF}}
La première version du notebook est centrée sur l'étude de \texttt{NLF} par rapport à \texttt{MMPose}.
Ces deux outils réalisent les mêmes objectifs, trouver les points d'un squelette sur des silhouttes animales, humains, ou de mains. Ici seulement les modèles sur les humains nous intéressent.
...
Elle reprend la logique de la \texttt{V2}, mais elle y ajoute un travail plus systématique sur l'analyse des résultats et sur les métriques de qualité, en plus d'une possibilité de tester n'importe quelle vidéo hors du dataset d'origine.
...
L'objectif est de tester \texttt{SAM 3} comme nouvelle approche pour la segmentation, d'étudier son comportement sur la vidéo principale et d'évaluer s'il peut remplacer ou compléter la stratégie actuelle.
Cette version de \texttt{SAM} est récente et a été postée durant la création des autres versions, et n'a donc pas été directement incluse dans notre stratégie globale.
