...
Ainsi, dans notre pipeline, l'analyse de pose n'est pas utilisée comme finalité en soi, mais comme base d'informations pour guider une méthode de segmentation plus adaptée au détouré précis de la danseuse.

\newpage

\section{Segmentation vidéo et reconstruction : implémentation principale (V3)}
La segmentation vidéo repose sur l'utilisation d'un second outil : \texttt{SAM}. Il permet de détourer des formes, notamment de personnes, sur un flux photo ou vidéo.
Cet outil prend en entrée un ensemble de points à l'intérieur et à l'exétrieur de la forme désignée, et permet de détourer les contour de la forme. Le résultat obtenu est un calque binaire de la forme trouvée.
La contribution centrale du projet, dans son état actuel, repose alors sur la combinaison de \texttt{NLF} et de \texttt{SAM 2}.
...
L'outil \texttt{SAM} utilisé seul sur notre vidéo témoin possède des résultats moyennement convaincants.
...
On peut assez facilement remarquer que \texttt{SAM} perd le contexte de la forme quand il est lancé seul, il réussit à garder une forme correcte environ une dizaine de secondes.

La sortie de \texttt{NLF}, c'est-à-dire l'ensemble de joints 2D trouvés, nous permet de contourner ce problème : en les confiant à \texttt{SAM} pour qu'il dessine un forme à partir de ceux-ci, les résultats sont plus stables.
En termes de temps d'exécution, \texttt{NLF} et \texttt{SAM} sont raisonnables, pour \texttt{NLF} il faut environ $200$ms pour $1$ frame ($5$ frames par secondes), donc $4'43''$ sur $1329$ frames pour la vidéo témoin.
Quant à lui, \texttt{SAM} est 7 fois plus lent environ pour une frame, soit $0{,}6$ frames par seconde. Ménager \texttt{SAM} permet de gagner du temps.

On peut donc considérer la situation comme un curseur qui oscille entre l'usage de \texttt{NLF} et \texttt{SAM} dont les critères sont la qualité des calques et le temps d'exécution.
...
De plus \texttt{SAM} réagit moins bien quand la série de frame est trop courte. et \texttt{NLF} a tendance à faire apparaitre la barre quand la segmentation est recalculée.
...
Au niveau du temps d'éxécution, on est autour de 17 minutes pour $1329$ frames ($0.767$ frames par secondes), la durée pouvant atteindre un peu moins de 20 minutes avec \texttt{SAM} uniquement.
...
Il aurait pu être envisager de combiner dans la stratégie les frames de transitions entre les deux outils, permettant peut-être de trouver le meilleur des deux mondes avec une approche combinatoire. Nous n'avons pas creuser cette piste, les résultats étant déjà satisfaisants.
...
Un aspect important de la version \texttt{V3} du projet est l'analyse consacré à l'évaluation quantitative des masques générés.
